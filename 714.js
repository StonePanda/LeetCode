...
//     // 这道题之前做过不包含手续费的买卖股票
//     // 其实就是谷底买入，谷峰卖出
//     // 但是加了手续费，如果收益<=手续费那么就要舍弃
//     // 可以舍弃这次交易，但是前一个交易的buyin的点如果更低
//     // 肯定不能舍弃，所以变成暂且最小和暂时最大交易
//     // 同时记得扣手续费
//     let profit=[]
...
//             sellout=prices[i]
//         }
//         if(sellout-buyin-fee>0){
//             profit.push([buyin,sellout])
//             buyin=sellout
//             sellout=0
//         }
//     }
...
//             sumprofit+=profit[i][1]-profit[i][0]-fee
//             break
//         }
//         if(profit[i+1][1]>profit[i][1])
//             // 只有后面的卖出点比较高的时候才会出现这种情况
//             // 卖两次没有卖一次的多
//             if((profit[i+1][1]-profit[i+1][0]-fee+profit[i][1]-profit[i][0]-fee)<(profit[i+1][1]-profit[i][0]-fee)){
//                 sumprofit+=profit[i+1][1]-profit[i][0]-fee
//                 i++
//             }            
//             else sumprofit+=profit[i][1]-profit[i][0]-fee
//         else sumprofit+=profit[i][1]-profit[i][0]-fee
//     }
//     return sumprofit
// };
...
var maxProfit = function(prices, fee) {
    ...
    // 那么我肯定可以在7的时候卖出，但是不能真正卖出，如果后面
    // 有更好的卖出点的话我应该在后面的点卖出
    // 但不一定有
    // 所以就出现了，这个卖出点不一定是最好的卖出点
    // 理论上来说，没有循环的时候，每个卖出点都是这种情况
    // 所以怎么让他给后面做准备呢
    // 那就是卖出这个的时候,计算收益
    // 同时买入，且买入的点是prices[i]-fee
    // 这样之后遇到了更好的卖出点,收益就是
    // sellout1-buyin-fee+(sellout2-sellout1+fee-fee)
    // =sellout2-buyin-fee
